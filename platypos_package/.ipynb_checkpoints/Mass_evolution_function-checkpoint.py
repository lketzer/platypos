...
from Mass_loss_rate_function import mass_loss_rate_forward_LO14, mass_loss_rate_forward_Ot20, mass_loss_rate_forward_MESA
import numpy as np
import math
...
def mass_planet_RK4_forward_MESA(epsilon, K_on, beta_on, planet_object, initial_step_size, t_final, track_dict):
    """USED: Runge-Kutta method to numerically integrate an ordinary differential equation by using a trial step at the 
    midpoint of an interval to cancel out lower-order error terms.
    -> Integrate from the current time (t_start (where planet has R0 and M0) into the future. 
    *********I have sth like: Mdot = f(M_pl, Fxuv), and I want M_pl(t)*********
    ...
    t_final: final time of simulation
    """ 
    interp_R_dict = planet_object.R_interpolation_functions
    ...
    R0 = R = planet_object.radius # should match observed radius
    M0 = M = planet_object.mass # planetary mass estimate based on mass-radius relation
    rho0 = rho = plmoMESA.density_planet(M0, R0)  # initial approx. density
    ...
    # CRITERION when to stop the mass-loss
    ################
    R_stop = 3 # stop when this radius is reached! (3 is an arbitrary number)
    M_core = 10.
    # if this criterium is reached, we consider the atmosphere to be completely evaporated
    
    # the other stopping criterium for MESA planets is when a lower-mass planet evolves off the available mass-grid
    # in this case we flag the planet by setting evolved_off_grid = True
    # then the planet_class_MESA function evolve planet can know if this is the reason for termination, 
    # and can create an extra file in the planet folder which can later be used to identify and remove such planets
    # from the population
    evolved_off_grid = False
    ...
    while t <= t_final:
        ...
        ####
        # I could make the code below shorter!!
        
        # for first step use the parameters initialized above (beta=beta0, K=K0, M=M0)
        ###############################################################################################################
        Mdot1 = mass_loss_rate_forward_MESA(t, epsilon, K_on, beta_on, planet_object, R, M, track_dict) # mass M, radius R
        k1 = (dt*Myr_to_sec * Mdot1)/M_earth # mass lost in one timestep in earth masses
        # check if this new planet would still be larger than core radius!
        if np.iscomplex(k1):
            print("1.STOP this madness! Leaving volatile regime!")
            # if the stop criterium is reached, I add the core mass & core radius to the array at t_i+1
            print("t = ", t_arr[-1]+dt)
            t_arr.append(t_arr[-1]+dt)
            M_arr.append(plmoMESA.calculate_mass_planet_MESA(R_stop, planet_object.distance, t_arr[-1]+dt, interp_R_dict))
            R_arr.append(R_stop)
            Lx_arr.append(Lx_evo(t=t_arr[-1]+dt, track_dict=track_dict))            
            return np.array(t_arr), np.array(M_arr), np.array(R_arr), np.array(Lx_arr), evolved_off_grid
        ###############################################################################################################
        M_05k1 = M + 0.5*k1  
        if M_05k1 < 0.15:
            # planet mass outside of grid!! -> Terminate
            print("New mass outside of M-R grid. STOP!")
            print("t = ", t_arr[-1]+dt)   
            evolved_off_grid = True
            return np.array(t_arr), np.array(M_arr), np.array(R_arr), np.array(Lx_arr), evolved_off_grid
        
        R_05k1 = plmoMESA.calculate_radius_planet_MESA(M_05k1, planet_object.core_mass, planet_object.distance, t+0.5*dt, interp_R_dict)
        
        Mdot2 = mass_loss_rate_forward_MESA(t+0.5*dt, epsilon, K_on, beta_on, planet_object, R_05k1, M_05k1, track_dict)
        #R_p=radius_planet(M + 0.5*k1))
        k2 = (dt*Myr_to_sec * Mdot2)/M_earth
        
        if np.iscomplex(k2):
            print("2.STOP this madness! Leaving volatile regime!")
            # if the stop criterium is reached, I add the core mass & core radius to the array at t_i+1
            print("t = ", t_arr[-1]+dt)
            t_arr.append(t_arr[-1]+dt)
            M_arr.append(plmoMESA.calculate_mass_planet_MESA(R_stop, planet_object.distance, t_arr[-1]+dt, interp_R_dict))
        ...
        R_05k2 = plmoMESA.calculate_radius_planet_MESA(M_05k2, planet_object.core_mass, planet_object.distance, t+0.5*dt, interp_R_dict)
        
        Mdot3 = mass_loss_rate_forward_MESA(t+0.5*dt, epsilon, K_on, beta_on, planet_object, R_05k2, M_05k2, track_dict) 
        # R_p=radius_planet(M + 0.5*k2))
        k3 = (dt*Myr_to_sec * Mdot3)/M_earth
        if np.iscomplex(k3):
            print("3.STOP this madness! Leaving volatile regime!")
            # if the stop criterium is reached, I add the core mass & core radius to the array at t_i+1
            print("t = ", t_arr[-1]+dt)
            t_arr.append(t_arr[-1]+dt)
            M_arr.append(plmoMESA.calculate_mass_planet_MESA(R_stop, planet_object.distance, t_arr[-1]+dt, interp_R_dict))
        ...
        R_05k3 = plmoMESA.calculate_radius_planet_MESA(M_05k3, planet_object.core_mass, planet_object.distance, t+dt, interp_R_dict)
        
        Mdot4 = mass_loss_rate_forward_MESA(t+dt, epsilon, K_on, beta_on, planet_object, R_05k3, M_05k3, track_dict) 
        # R_p=radius_planet(M + k3))
        k4 = (dt*Myr_to_sec * Mdot4)/M_earth
        if np.iscomplex(k4):
            print("4.STOP this madness! Leaving volatile regime!")
            # if the stop criterium is reached, I add the core mass & core radius to the array at t_i+1
            print("t = ", t_arr[-1]+dt)
            t_arr.append(t_arr[-1]+dt)
            M_arr.append(plmoMESA.calculate_mass_planet_MESA(R_stop, planet_object.distance, t_arr[-1]+dt, interp_R_dict))
        ...
        # if radius change is too drastic, decrease step size
        M_new = M + M_lost
        #print(M_new, planet_object.distance, t_arr[-1] + dt)
        R_new = plmoMESA.calculate_radius_planet_MESA(M_new, planet_object.core_mass, planet_object.distance, t_arr[-1] + dt, interp_R_dict) # at new timestep
        #print("R_next = ", R_new)
        ...
        if (abs((R-R_new)/R)*100 >= 1.) & (t < track_dict["t_curr"]) & (dt > 1e-2):#10**(-1)): # smaller than 1%
            ...
            if ((M + M_lost) - M_core) >= 0:
                ...
                # calculate new radius with new planet mass & one time step later
                R = plmoMESA.calculate_radius_planet_MESA(M, planet_object.core_mass, planet_object.distance, t, interp_R_dict) # at new timestep
                #print("new:", M, R, t)
                R_arr.append(R)
                i = i+1 # update step to i+1
                
            elif plmoMESA.calculate_radius_planet_MESA(M+M_lost, planet_object.core_mass, planet_object.distance, t_arr[-1] + dt, interp_R_dict): 
                # planet mass outside of grid!! -> Terminate
                print("New mass outside of M-R grid. STOP!")
                print("t = ", t_arr[-1]+dt)   
                evolved_off_grid = True
