...
from platypos.lx_evo_and_flux import lx_evo_ext
...
def mass_evo_RK4_forward(planet_object,
    ...
    step_size (float): initial step_size, variable, unless fixed_step_size=True
    ...
    fenv_sample_cut (bool): keep fenv constant at 0.01% once calculation
            hits this limit (do not extrapolate beyond the lower limit of 
            the model); in final planetary stages, this basically doesn't
            make a difference - planet will quickly lose its last bit of 
            envelope once it hits this limit.
            Default is True;
    
    fixed_step_size (bool): adjustable step size yes or no
            Default is False; if set to True, the whole calcluation will
            proceed with the same chosen step size and does not adjust
            automatically.
            
            
    forward_backward (bool):
    
    t_start (float): start time of simulation (in Myr)
    ...
    if forward_backward == True:
        Lx0 = lx_evo_ext(t=t_start, track_dict=track_dict, 
    ...
    if fixed_step_size is True:
        # create time array for integration (with user-specified step size)
        number = math.ceil((t_final-t0)/initial_step_size)
        times, step_size2 = np.linspace(t0, t_final, number-1,
                                        endpoint=False, retstep=True)
        #print(times)
        dt = step_size2
        #print(initial_step_size, dt)
        min_step_size = max_step_size = dt
    ...
    while t <= t_final:
        ...
        if forward_backward == True:
            Lx_i = lx_evo_ext(t=t, track_dict=track_dict, 
                         forward_backward=forward_backward)
        ...
        if ((t + dt) > t_final) & (t < t_final) &  ((t + dt) != t_final):
            dt_last = dt = t_final - t
            #print('here', dt)
            close_to_evaporation = True
            weird_last_step_case = True
        ...
        while (envelope_left == True):
            ...
            if ((np.isnan(M_lost) == True) \
                ...
                R_arr.append(R_core)
                if forward_backward == True:
                    Lx_arr.append(lx_evo_ext(t=t_arr[-1]+dt, track_dict=track_dict,
            ...
            if (close_to_evaporation == False):
                ...
                if (R_change > 0.5) \
                    ...
                    if fixed_step_size is False:
                        ...
                        print("R-change drastic for chosen step-size.")
                ...
                #    break
                    ...
                    if fixed_step_size is False:
                        ...
                        print("R-change too small for chosen step-size.")
                ...
                # evolution where almost no change is occurring anymore
                    ...
                    if ((M + M_lost) - M_core) > 0: 
                        ...
                        t_arr.append(t)
                        if forward_backward == True:
                            Lx_arr.append(lx_evo_ext(t=t, track_dict=track_dict,
                                                    forward_backward=True))
                        else:
    ...
    #print(weird_last_step_case)
    if weird_last_step_case is True:
        #print('0')
        return pd.DataFrame({"Time": t_arr[:-1], "Mass": M_arr[:-1],
                         "Radius": R_arr[:-1], "Lx": Lx_arr[:-1],
                         "Mdot_info": Mdot_info[:-1]})
    else:
        if (t_arr[-1] > t_final):
            #print('1')
            return pd.DataFrame({"Time": t_arr[:-1], "Mass": M_arr[:-1],
...
def mass_evo_RK4_backwards(planet_object,
                         track_dict,
                         mass_loss_calc="Elim",
                         epsilon=None,
                         K_on="yes", beta_settings={'beta_calc': 'off'},
    ...
    # convert X-ray to XUV lum. and calculate planet's high energy 
    # incident flux
    Lx0 = lx_evo_ext(t=track_dict["t_start"], track_dict=track_dict,
                     forward_backward=True)
    Lxuv0 = l_xuv_all(Lx0, relation_EUV, planet_object.mass_star)
    Fxuv0 = flux_at_planet(Lxuv0, planet_object.distance)
    ...
    if fixed_step_size is True:
        # create time array for integration (with user-specified step size)
        number = math.ceil(abs(t_final-t0)/initial_step_size)
        times, step_size2 = np.linspace(t0, t_final, number-1,
                                        endpoint=False, retstep=True)
        dt = step_size2
        #print(-initial_step_size, dt)
    ...
    stop_conditions_fulfilled = False
    weird_last_step_case = False    
    ...
    while t >= t_final:
        ...
        # embedded in the mass_loss_rate_fancy function)
        #Lx_i = lx_evo(t=t, track_dict=track_dict)
        Lx_i = lx_evo_ext(t=t, track_dict=track_dict, forward_backward=True)
        #print(i, j, " t=", t, dt, "Lx=", Lx_i)
        # IMPORTANT points on the time step:
        ...
        #print(t, dt, t_final, '-', (t + dt) < t_final, (t > t_final), ((t + dt) != t_final))
        if ((t + dt) < t_final) & (t > t_final) & ((t + dt) != t_final):
            dt_last = dt = t_final - t
            #print('here', -dt)
            weird_last_step_case = True
        ...
        while stop_conditions_fulfilled is False:
#             print('RK-loop', t, np.round(dt,4))
            # go through RK iterations as long as stop conditions are not fulfilles

            # apply Runge Kutta 4th order to find value of M_dot
            # NOTE: the mass lost in one timestep is in Earth masses
            if ((planet_object.planet_type == "LoFo14") or
               (planet_object.planet_type == "ChRo16")):
                if (fenv_sample_cut == True) & (f_env < 1e-2):\
            ...
            k4 = (dt * Myr_to_sec * Mdot4) / M_EARTH
            # total mass gained after time-step dt
            ...
            M_new = M + M_lost
            M_env_new = M_new - M_core
#             print(t, np.round(dt,4), M_lost, M_new)

        
            # check, if planet has reached conditions to stop
            # stop_conditions_fulfilled == True
            # e.g. values of fenv drastically outside the range
            # of acceptable values for the planet models (e.g. 40%)
...
#             print("R_new, f_new: ", R_new, f_env_new)
            ...
            if (weird_last_step_case is False):
                R_change = abs((R - R_new) / R) * 100 # radius change compared to
                                                # previous radius - in percent
                
                if (R_change > 0.5) and (dt < min_step_size) and (fixed_step_size is False): #and (t < track_dict["t_curr"]) \
                    dt = dt / 10.
                    j += 1
                    break
                    
                elif (R_change < (0.02)) and (dt > max_step_size) and (fixed_step_size is False): #and (t < track_dict["t_curr"]) \
                    dt = dt * 10.
                    j += 1
                    break    
                
                else: # if radius change is ok, or fixed_step_size is True
                    # do sanity check: is new planet mass is greater? 
                    # in this case update params and go into next RK iteration
                    if ((M + M_lost) - M) > 0: 
                        M = M + M_lost  # new planet mass (M_lost is negative)
                        t = t_arr[-1] + dt  # updated time value t_i_plus_1
            ...
            if (weird_last_step_case is True):
#                 print('weird_last_step_case is True')
                if ((M + M_lost) - M_core) > 0: 
                    M = M + M_lost  # new planet mass (M_lost is negative)
                    t = t_arr[-1] + dt  # updated time value t_i_plus_1
                    M_arr.append(M)
                    ...
                    return pd.DataFrame({"Time": t_arr, "Mass": M_arr,
                         ...
                         "Mdot_info": Mdot_info})
                    # this should never happen
                    sys.exit('Xsth went wrong of you see this!')
                
            break
            
        if (stop_conditions_fulfilled == True):
            print("stop_conditions_fulfilled")
            # planet has evaporated, so return last planet params for bare core
            Mdot_info.append('End')
            return pd.DataFrame({"Time": t_arr, "Mass": M_arr,
                                 "Radius": R_arr, "Lx": Lx_arr,
        ...
        # check how drastic the radius change is an adjust accordingly
        
        # drastic change, make step size smaller
        
        # small change, make step size bigger
